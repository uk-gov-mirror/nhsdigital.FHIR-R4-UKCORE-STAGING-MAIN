...
Where they don't already exist, the UK Core approach to creating codes for concepts in a new CodeSystem is by placing a hyphen between each word of the `code` value (if they contain more than one word) and using solely lower case alphabetic characters. For example:
...
All codes SHOULD have a `definition`, although this is not always possible for legacy codes. See <a href="https://hl7.org/fhir/R4/codesystem.html#designations" target="_blank">HL7 FHIR - Display, Definition and Designations</a> for more information.
